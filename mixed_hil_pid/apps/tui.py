...
from textual.widgets import (
    ...
    DataTable, Select, TabbedContent, TabPane, Markdown
)
...
class ConfigEditorScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with ScrollableContainer():
            ...
            yield Static("Edit PID optimization parameters below. Press Ctrl+S to save, Esc to cancel.", 
                        classes="help-text")
            
            # PID Bounds
            yield Static("\n📐 PID Bounds", classes="subsection-title")
            with Vertical(classes="config-group"):
                for i, (name, bounds) in enumerate([("Kp", self.config.get("pid_bounds", [[0.1, 10], [0.01, 10], [0.01, 10]])[0]),
                                                     ("Ki", self.config.get("pid_bounds", [[0.1, 10], [0.01, 10], [0.01, 10]])[1]),
                                                     ("Kd", self.config.get("pid_bounds", [[0.1, 10], [0.01, 10], [0.01, 10]])[2])]):
                    with Horizontal(classes="input-row"):
                        yield Label(f"{name} Range:", classes="input-label")
                        inp_min = Input(value=str(bounds[0]), placeholder=f"Min {name}", classes="config-input")
                        inp_max = Input(value=str(bounds[1]), placeholder=f"Max {name}", classes="config-input")
            ...
            with Vertical(classes="config-group"):
                for key, label in [
                    ...
                    with Horizontal(classes="input-row"):
                        ...
                        inp = Input(value=str(self.config.get(key, 0)), placeholder=label, classes="config-input")
                        self.inputs[key] = inp
                        yield inp
            ...
            with Vertical(classes="config-group"):
                for key, label in [
                    ...
                    with Horizontal(classes="input-row"):
                        yield Label(f"{label}:", classes="input-label")
                        inp = Input(value=str(self.config.get(key, 1 if key == "num_experiments" else 0)), placeholder=label, classes="config-input")
                        self.inputs[key] = inp
                        yield inp
            
            # Action buttons
            with Horizontal(classes="button-row"):
                yield Button("💾 Save Configuration", variant="success", id="save-btn")
    ...
    def action_save_config(self) -> None:
        ...
        try:
            ...
            self.app.notify("✅ Configuration updated! Changes will apply on next run.", severity="information")
            self.app.pop_screen()
...
class HILApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with ScrollableContainer():
            # Welcome
            yield Static("🚀 Mixed HIL-PID Optimization Suite", classes="section-title")
            yield Static("Choose your optimization approach and configure parameters", classes="help-text")
    ...
    def render_approaches(self) -> ComposeResult:
        ...
        for approach in approaches:
            with Container(classes="approach-card"):
                ...
                yield Button(f"▶️  Run {approach['id'].upper()}", 
    ...
    def render_config_preview(self) -> ComposeResult:
        ...
        with Container(classes="config-preview"):
            ...
            table.add_row("PID Bounds (Kp)", f"{self.current_config['pid_bounds'][0]}")
            table.add_row("PID Bounds (Ki)", f"{self.current_config['pid_bounds'][1]}")
            table.add_row("PID Bounds (Kd)", f"{self.current_config['pid_bounds'][2]}")
            table.add_row("Simulation Steps", f"{self.current_config['simulation_steps']}")
            table.add_row("Target Yaw", f"{self.current_config['target_yaw_deg']}°")
            table.add_row("Max Overshoot", f"{self.current_config['pid_max_overshoot_pct']}%")
            table.add_row("Max Rise Time", f"{self.current_config['pid_max_rise_time']}s")
            table.add_row("Max Settling Time", f"{self.current_config['pid_max_settling_time']}s")
            table.add_row("Max Iterations", f"{self.current_config['max_iterations']}")
            table.add_row("DE Mutation Factor", f"{self.current_config['base_mutation']}")
            table.add_row("BO PoF Min", f"{self.current_config['bo_pof_min']}")
            
            
            yield table
            yield Button("✏️  Edit Configuration", variant="primary", id="edit-config-btn")
    ...
    def save_config_to_file(self, config_file: Path) -> None:
        ...
        yaml_config = {
            ...
            'pid_max_overshoot_pct': int(self.current_config['pid_max_overshoot_pct']),
