...
def main(num_experiments=1, robot_type="husky"):
    ...
    try:
        ...
        global STATE_PATH
        STATE_PATH = Path(f"logs/{robot_type}_logs/BO_HIL_{robot_type}") / "rerun_state.json"
        ...
        for run_idx in range(start_run, num_experiments + 1):
            print(f"\n{'='*60}\n  BO HIL ({robot_type}): Experiment {run_idx}/{num_experiments}\n{'='*60}")
            
            best_path = executor.run_bo_hil_experiment(run_idx, num_experiments, batch_id, robot_type)
