...
STATE_PATH = Path("logs/DE_autorun") / "rerun_state.json"


def main(num_experiments=1):
    """
    Run multiple DE vanilla (autorun) experiments.
    ...
    try:
        ...
        for run_idx in range(start_run, num_experiments + 1):
            print(f"\n{'='*60}\n  DE Autorun: Experiment {run_idx}/{num_experiments}\n{'='*60}")
            
            best_path = executor.run_de_vanilla_experiment(run_idx, num_experiments, batch_id)
