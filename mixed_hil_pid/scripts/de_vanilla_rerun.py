#!/usr/bin/env python3
"""
DE Vanilla Rerun: Execute Multiple DE Autorun Experiments

Runs multiple automated Differential Evolution experiments without human feedback.
These are "vanilla" experiments that run automatically for benchmarking purposes.
...
Usage: 
    python de_vanilla_rerun.py [num_experiments]
    
Example:
    python de_vanilla_rerun.py 5  # Run 5 experiments
"""
...
def main(num_experiments=1, robot_type="husky"):
    """
    Run multiple DE vanilla (automated) experiments.
    ...
    try:
        ...
        global STATE_PATH
        STATE_PATH = Path(f"logs/{robot_type}_logs/DE_Vanilla_{robot_type}") / "rerun_state.json"
        ...
        for run_idx in range(start_run, num_experiments + 1):
            print(f"\n{'='*60}\n  DE Vanilla ({robot_type}): Experiment {run_idx}/{num_experiments}\n{'='*60}")
            
            best_path = executor.run_de_vanilla_experiment(run_idx, num_experiments, batch_id, robot_type)
            save_state(STATE_PATH, "DE", batch_id, run_idx, num_experiments)
            
            print(f"\nExperiment {run_idx} completed. Best results: {best_path}")
        
        print(f"\n✓ All {num_experiments} DE Vanilla experiments completed!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run multiple DE Vanilla (autorun) experiments")
    parser.add_argument(
        "num_experiments", 
        type=int, 
        nargs="?", 
