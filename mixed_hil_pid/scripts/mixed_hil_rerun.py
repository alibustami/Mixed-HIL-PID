#!/usr/bin/env python3
"""
Mixed HIL Rerun: Execute Multiple Mixed HIL Experiments

Runs multiple human-in-the-loop PID optimization experiments using the Mixed approach
(combining Differential Evolution and Bayesian Optimization with human feedback).
...
Usage: 
    python mixed_hil_rerun.py [num_experiments]
    
Example:
    python mixed_hil_rerun.py 5  # Run 5 experiments
"""
...
def main(num_experiments=1, robot_type="husky"):
    """
    Run multiple Mixed HIL experiments.
    ...
    try:
        ...
        global STATE_PATH
        STATE_PATH = Path(f"logs/{robot_type}_logs/MixedHIL_{robot_type}") / "rerun_state.json"
        
        # Resolve state for resumption
        batch_id, start_run = resolve_state(STATE_PATH, "Mixed", num_experiments)
        save_state(STATE_PATH, "Mixed", batch_id, start_run - 1, num_experiments)
        ...
        for run_idx in range(start_run, num_experiments + 1):
            print(f"\n{'='*60}\n  Mixed HIL ({robot_type}): Experiment {run_idx}/{num_experiments}\n{'='*60}")
            
            best_path = executor.run_mixed_hil_experiment(run_idx, num_experiments, batch_id, robot_type)
            save_state(STATE_PATH, "Mixed", batch_id, run_idx, num_experiments)
            
            print(f"\nExperiment {run_idx} completed. Best results: {best_path}")
        
        print(f"\n✓ All {num_experiments} Mixed HIL experiments completed!")
