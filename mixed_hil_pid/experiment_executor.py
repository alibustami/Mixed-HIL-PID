...
from mixed_hil_pid.optimizers.differential_evolution import DifferentialEvolutionOptimizer
from mixed_hil_pid.optimizers.bayesian_optimizer import ConstrainedBayesianOptimizer
from mixed_hil_pid.metrics import calculate_metrics, meets_pid_targets, violation_from_sat
from mixed_hil_pid.logger import init_logger, log_iteration, append_histories_pickle
...
class ExperimentExecutor:
    ...
    def run_mixed_hil_experiment(self, run_index, total_runs, batch_id, robot_type="husky"):
        ...
        de = DifferentialEvolutionOptimizer(bounds=PID_BOUNDS, pop_size=6, mutation_factor=_CONFIG['base_mutation'])
        de.population[0] = np.clip(pref_model.anchor_params(), de.bounds[:, 0], de.bounds[:, 1])
        bo = ConstrainedBayesianOptimizer(bounds=PID_BOUNDS, pof_min=_CONFIG['bo_pof_min'])
        gui = MixedHILGUI()
            ...
            while iteration < _CONFIG['max_iterations']:
                ...
                for lbl, c, f, v, m in [("DE", cand_a, fit_a, viol_a, metrics_a), ("BO", cand_b, fit_b, viol_b, metrics_b)]:
                    if better_candidate(f, v, best_record):
                        best_record = {"iteration": iteration, "label": lbl, "params": c.tolist(), "fit": float(f), "violation": float(v), "metrics": m}
                ...
                if (target_ok_a and viol_a <= 0) or (target_ok_b and viol_b <= 0):
                    ...
                    log_iteration(log_path, iteration, "auto_terminate", cand_a, fit_a, viol_a, sat_a, metrics_a, target_ok_a,
                                 ...
                                 pref_model.weights, "auto_term", time.time() - iter_start)
                    break
                
                # GUI feedback
                choice = gui.show_comparison(hist_a, hist_b, cand_a, cand_b, metrics_a, metrics_b)
                choice_labels = {1: "prefer_de", 2: "prefer_bo", 3: "tie_refine", 4: "reject_expand"}
                
                if choice == 1:  # Prefer DE
                    pref_model.update_towards(cand_a, cand_b)
                    de.inject_candidate(pref_model.anchor_params(), eval_func=fitness_wrapper, protect_best=True)
                ...
                log_iteration(log_path, iteration, choice_labels.get(choice, "exit"), cand_a, fit_a, viol_a, sat_a, metrics_a, target_ok_a,
                             cand_b, fit_b, viol_b, sat_b, metrics_b, target_ok_b, de.mutation_factor, *de.best_scores(),
                             np.mean(np.std(de.population, axis=0)), min(fit_a, fit_b), bo.bounds[:, 1] - bo.bounds[:, 0],
                             pref_model.weights, choice_labels.get(choice, ""), time.time() - iter_start)
        ...
        try:
            while iteration < _CONFIG['max_iterations']:
                ...
                if choice == 1:  # ACCEPT (Refine)
                    ...
                    print(f"[Accept] Refining around Kp={cand[0]:.2f}, Ki={cand[1]:.2f}, Kd={cand[2]:.2f}")
    ...
    def run_bo_vanilla_experiment(self, run_index, total_runs, batch_id, robot_type="husky"):
        ...
        lows, highs = np.array([b[0] for b in PID_BOUNDS]), np.array([b[1] for b in PID_BOUNDS])
        ...
        try:
            while iteration < _CONFIG['max_iterations']:
                ...
                # Update best
                if best_record is None or (safe_ok and fit < best_record["fit"]):
                    best_record = {
                        "iteration": iteration,
                        "params": cand.tolist(),
                        "fit": float(fit),
                ...
                bo_spans = bo.bounds[:, 1] - bo.bounds[:, 0]
                log_iteration(
                    log_path, iteration, "auto",
                    cand, fit, violation, sat, metrics, target_ok,
                    cand, fit, violation, sat, metrics, target_ok,
                    0.0, float("inf"), float("inf"), 0.0, best_overall_fit,
                    bo_spans, np.zeros(3), "BO_autorun", time.time() - iter_start
                )
                
                append_histories_pickle(pkl_path, [{
                    "iteration": iteration, "label": "BO", "params": cand.tolist(),
                    "fit": float(fit), "violation": float(violation),
                    "metrics": metrics, "history": hist,
                }])
        ...
        try:
            while iteration < _CONFIG['max_iterations']:
                ...
                log_iteration(
                    ...
                    cand, fit, violation, sat, metrics, target_ok,
                    de.mutation_factor, de_best_fit, de_best_viol,
                    np.mean(np.std(de.population, axis=0)), best_overall_fit,
                    np.zeros(3), np.zeros(3), "DE_autorun", time.time() - iter_start
                )
