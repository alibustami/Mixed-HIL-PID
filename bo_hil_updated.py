...
MAX_ITERATIONS = 100
DISPLAY_REALTIME = False  # True to watch in realtime (slower)
TARGET_YAW_DEG = 90.0
...
def bullet_worker(req_q: mp.Queue, resp_q: mp.Queue):
    ...
    def evaluate_pid(pid_params, label_text="", return_history=False, realtime=False):
        ...
        target_rad = float(np.deg2rad(TARGET_YAW_DEG))
...
def main():
    ...
    def run_sim(params, label_text="", return_history=False, realtime=False):
        ...
        while True:
            r = resp_q.get()
            if r["id"] == job_id:
                return float(r["fitness"]), r["history"], r["sat"]
    ...
    # --- helper for objective + constraint ---
    def eval_obj_and_violation(params):
        fit, _, sat = run_sim(params, return_history=False, realtime=False)
        viol = float(sat["max_abs_raw_output"] - PID_OUTPUT_LIMIT)  # <=0 feasible
        return float(fit), float(viol)

    # --- Init seeds (include conservative safe seed) ---
    safe_seed = np.array([b[0] for b in PID_BOUNDS], dtype=float)
    fit0, viol0 = eval_obj_and_violation(safe_seed)
    bo.update(safe_seed, fit0, viol0)
    print(f"[BO Init] safe_seed {safe_seed} -> fit={fit0:.4f}, viol={viol0:.3f}")

    lows = np.array([b[0] for b in PID_BOUNDS], dtype=float)
    highs = np.array([b[1] for b in PID_BOUNDS], dtype=float)
    for i in range(INIT_BO_SEEDS):
    ...
    try:
        while iteration < MAX_ITERATIONS:
            ...
            for attempt in range(1, BO_MAX_RETRIES + 1):
                ...
                print(f"[BO] Retry {attempt}/{BO_MAX_RETRIES}: infeasible viol={v_fast:.3f}")

            if cand is None:
                best = bo.best_feasible()
                if best is not None:
                    cand, _, _ = best
                    print("[BO] Fallback to best feasible point seen so far.")
                else:
                    cand = safe_seed
                    print("[BO] Fallback to safe_seed (no feasible points yet).")

            # 2) Evaluate chosen candidate with history for visualization + metrics
            fit, hist, sat = run_sim(
                cand,
            ...
            violation = float(sat["max_abs_raw_output"] - PID_OUTPUT_LIMIT)
